param([string[]]$Probes = @('chessis28', 'chessis27', 'chessis26', 'chessis25', 'chessis24', 'chessis16', 'chessis13', 'chessis11'), [switch]$CoreOnly)
...
$taskOutput = Join-Path $taskRoot 'review/app/chessis28'
New-Item -ItemType Directory -Force -Path $taskOutput | Out-Null
if ($CoreOnly) {
    foreach ($taskScript in @('motion_clock28_test', 'editor27_test', 'evaluation26_test', 'variation25_test', 'rules_test', 'chessis_core_test', 'historic_games_test', 'report12_test', 'story19_test', 'tournament_sync20_test')) {
        $taskLog = Join-Path $taskOutput ($taskScript + '.log')
        & $taskGodot --headless --path (Join-Path $taskRoot 'godot') --script "res://tests/$taskScript.gd" -- --chessis20-regression --chessis28-regression *> $taskLog
...
foreach ($taskProbe in $Probes) {
    ...
    $taskProcess = Start-Process -FilePath $taskGodot -ArgumentList @('--path', ('"' + (Join-Path $taskRoot 'godot') + '"'), '--', '--unified-test', ('--' + $taskProbe), '--chessis28-regression') -WindowStyle Hidden -RedirectStandardOutput $taskStdout -RedirectStandardError $taskStderr -PassThru
