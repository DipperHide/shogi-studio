...
$taskOutput = Join-Path $taskRoot 'review/app/chessis10'
New-Item -ItemType Directory -Force -Path $taskOutput | Out-Null
$taskScripts = @('report10_test')
if ($FullRegression) { $taskScripts += @('usi_test', 'chessis_core_test') }
foreach ($taskScript in $taskScripts) {
    & $taskGodot --headless --path (Join-Path $taskRoot 'godot') --script "res://tests/$taskScript.gd" *> (Join-Path $taskOutput ($taskScript + '.log'))
    if ($LASTEXITCODE -ne 0) { throw "$taskScript failed." }
    Get-Content -LiteralPath (Join-Path $taskOutput ($taskScript + '.log')) -Tail 1
}
$taskProbes = @('chessis10')
if ($FullRegression) { $taskProbes += 'chessis' }
foreach ($taskProbe in $taskProbes) {
    ...
    $taskProcess = Start-Process -FilePath $taskGodot -ArgumentList @('--path', ('"' + (Join-Path $taskRoot 'godot') + '"'), '--', '--unified-test', ('--' + $taskProbe), '--chessis10-regression') -WindowStyle Hidden -RedirectStandardOutput $taskStdout -RedirectStandardError $taskStderr -PassThru
