...
$taskOutput = Join-Path $taskRoot 'review/app/chessis17'
New-Item -ItemType Directory -Force -Path $taskOutput | Out-Null
if (-not $UiOnly) {
    $taskCore = if ($RegressionOnly) { @('report10_test', 'report12_test') } else { @('accuracy17_test', 'accuracy17_gote_test') }
    foreach ($taskScript in $taskCore) {
        & $taskGodot --headless --path (Join-Path $taskRoot 'godot') --script "res://tests/$taskScript.gd" -- --chessis17-regression *> (Join-Path $taskOutput ($taskScript + '.log'))
        if ($LASTEXITCODE -ne 0) { throw "$taskScript failed." }
        Get-Content -LiteralPath (Join-Path $taskOutput ($taskScript + '.log')) -Tail 1
    }
}
$taskProbes = if ($Probes) { $Probes } elseif ($RegressionOnly) { @('chessis12', 'chessis16', 'chessis15', 'chessis') } else { @('chessis17') }
foreach ($taskProbe in $taskProbes) {
    ...
    $taskProcess = Start-Process -FilePath $taskGodot -ArgumentList @('--path', ('"' + (Join-Path $taskRoot 'godot') + '"'), '--', '--unified-test', ('--' + $taskProbe), '--chessis17-regression') -WindowStyle Hidden -RedirectStandardOutput $taskStdout -RedirectStandardError $taskStderr -PassThru
