param([switch]$UiOnly, [string[]]$Probes = @('chessis22', 'chessis21', 'chessis12', 'chessis16', 'chessis19'))
...
$taskOutput = Join-Path $taskRoot 'review/app/chessis22'
New-Item -ItemType Directory -Force -Path $taskOutput | Out-Null
if (-not $UiOnly) {
    & $taskGodot --headless --path (Join-Path $taskRoot 'godot') --script res://tests/report_chart22_test.gd *> (Join-Path $taskOutput 'chart-core.log')
    if ($LASTEXITCODE -ne 0) { throw 'Chart geometry tests failed.' }
    Get-Content -LiteralPath (Join-Path $taskOutput 'chart-core.log') -Tail 1
}
foreach ($taskProbe in $Probes) {
    $taskStdout = Join-Path $taskOutput ($taskProbe + '.log')
    $taskStderr = Join-Path $taskOutput ($taskProbe + '.err')
    $taskProcess = Start-Process -FilePath $taskGodot -ArgumentList @('--path', ('"' + (Join-Path $taskRoot 'godot') + '"'), '--', '--unified-test', ('--' + $taskProbe), '--chessis22-regression') -WindowStyle Hidden -RedirectStandardOutput $taskStdout -RedirectStandardError $taskStderr -PassThru
