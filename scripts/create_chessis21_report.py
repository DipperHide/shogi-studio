"""Verify and summarize the 0.21 artifacts and the current successful test runs."""
...
OUT = ROOT / 'review/app/chessis21'
release = json.loads((OUT / 'release.json').read_text('utf-8'))
assert release['version'] == '0.21.0' and release['android_version_code'] == 39
proofs = {
    'ui/ui-tests.json': '阶段条布局、百分比边界、长姓名、触摸和键盘',
    'chessis12_test/results.json': '完整报告、筛选、阶段与准确率入口',
    'chessis17_test/results.json': '准确率详情、长图滚动与棋盘定位',
    'chessis18_test/results.json': '双方阶段详情、真实定式和唯一合法着',
...
for name in ['chessis21', 'chessis12', 'chessis17', 'chessis18', 'chessis16', 'chessis19']:
...
with ZipFile(ROOT / windows['path']) as archive:
    ...
    assert sha((ROOT / 'builds/windows-0.21.0/Shogi.exe').read_bytes()) == probe['executable_sha256']
with ZipFile(ROOT / apk['path']) as archive:
    ...
    compiled = {name: sha(archive.read('assets/scripts/' + name + '.gdc')) for name in [
        'shogi_report_phases', 'shogi_chessis_menu', 'shogi_package_probe']}
...
assert len(geometry) == 16
reference = ROOT / '.work/chessis-reference'
reference_files = ['res/layout/phase_ribbon.xml', 'res/drawable/bg_game_report_phase_side_group.xml',
                   'res/drawable/bg_game_report_phase_summary.xml', 'java/sources/p328r2/C4570h.java',
                   'java/sources/p304n5/AbstractC4378m.java']
...
(ROOT / 'docs/releases/v0.21.0-validation.json').write_text(json.dumps(validation, ensure_ascii=False, indent=2), 'utf-8')
...
共 **{sum(counts.values()):,} 项检查通过，0 项失败**。
...
成品验证确认 APK 为非 debuggable 的 release 构建、versionCode 39、原包名和相同签名、ARM64 与 16 KB 对齐。37 枚界面图标的源文件、导入纹理及 APK 内容核对一致，新阶段条字节码存在，会员资源缺席。Windows ZIP 的程序散列与实际运行的成品相同；首帧棋盘在 {probe['first_board_frame_ms']} 毫秒绘制。
...
(ROOT / 'docs/TESTING-0.21.md').write_text(report, 'utf-8')
shutil.copy2(OUT / 'ui/ribbon-dark-393x852.png', ROOT / 'docs/images/report-0.21.png')
checksums = ''.join(item['sha256'] + '  ' + Path(item['path']).name + '\n' for item in release['artifacts'])
(ROOT / 'builds/SHA256SUMS-0.21.0.txt').write_text(checksums, 'utf-8')
