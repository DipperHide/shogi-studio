...
$taskOutput = Join-Path $taskRoot 'review/app/chessis18'
New-Item -ItemType Directory -Force -Path $taskOutput | Out-Null
if (-not $UiOnly) {
    $taskCore = @('report12_test')
    foreach ($taskScript in $taskCore) {
        & $taskGodot --headless --path (Join-Path $taskRoot 'godot') --script "res://tests/$taskScript.gd" -- --chessis18-regression *> (Join-Path $taskOutput ($taskScript + '.log'))
        if ($LASTEXITCODE -ne 0) { throw "$taskScript failed." }
        Get-Content -LiteralPath (Join-Path $taskOutput ($taskScript + '.log')) -Tail 1
    }
}
$taskProbes = if ($Probes) { $Probes } elseif ($RegressionOnly) { @('chessis12', 'chessis17', 'chessis16') } else { @('chessis18') }
foreach ($taskProbe in $taskProbes) {
    ...
    $taskProcess = Start-Process -FilePath $taskGodot -ArgumentList @('--path', ('"' + (Join-Path $taskRoot 'godot') + '"'), '--', '--unified-test', ('--' + $taskProbe), '--chessis18-regression') -WindowStyle Hidden -RedirectStandardOutput $taskStdout -RedirectStandardError $taskStderr -PassThru
