"""Verify 0.26 release evidence without treating timing misses as functional passes."""
...
assert release['version'] == '0.26.0' and release['android_version_code'] == 44
proofs = {
    'variation-core.json': '嵌套变化、合法性、保存、转置局面与损坏数据',
    'evaluation-core.json': '评价条刻度、先后手、詰与设置持久化',
    'ui/ui-tests.json': '评价条布局、真实引擎、键盘、过渡和续下',
    'variations/ui-tests.json': '变化界面、实际触摸、备份、保存失败及真实引擎报告',
...
for name in ['chessis26', 'chessis25', 'chessis24', 'chessis16', 'chessis13', 'chessis11']:
...
with ZipFile(ROOT / apk['path']) as archive:
    ...
    compiled = {name: hashlib.sha256(archive.read(f'assets/scripts/{name}.gdc')).hexdigest() for name in [
        'shogi_app', 'shogi_chessis_menu', 'shogi_evaluation_bar', 'shogi_game', 'shogi_variation_tree', 'shogi_variation_study', 'shogi_variation_view']}
...
validation = {
    'version': '0.26.0', 'functional_checks': counts, 'functional_total': sum(counts.values()), 'failures': [],
    ...
    'artifacts': release['artifacts'], 'compiled_android_scripts': compiled, 'reference_evidence': read(OUT / 'reference-evidence.json'),
    'runtime_sources_match_release_receipt': True, 'windows_zip_matches_tested_executable': True,
}
first_probe = read(OUT / 'package-first-attempt/windows-package-probe.json')
assert first_probe['executable_sha256'] == probe['executable_sha256']
validation['initial_package_probe_failures'] = first_probe['failures']
validation['package_retest_conditions'] = 'Same binary; rerun after core replay and build/package processes finished.'
for destination in [OUT / 'artifact-checks.json', ROOT / 'docs/releases/v0.26.0-validation.json']:
...
原有回放的 36 次中途切换保持 40 枚棋子的姿态连续。22 段计时中仍有 **{len(missed)} 段超过 50 毫秒目标**，最大 **{max(row['max_gap'] for row in timing):.3f} 毫秒**；本轮动画探针在构建和核心回归前执行；这些是 Windows 测试环境的观测值，不能据此推断 Android 表现。上一版空窗口也出现绘制停顿，原因仍未解决，见 [动画衔接说明](ANIMATION-CONTINUITY.md)。

Windows 成品通过 {counts['package/windows-package-probe.json']} 项检查，首次棋盘绘制 {probe['first_board_frame_ms']} 毫秒。APK 使用 release 模板、versionCode 44、原包名与开发签名，ARM64／16 KiB 对齐；没有会员模块或测试夹具。发布文件哈希、源文件清单与受测 Windows 程序已校验。
...
'''
for destination in [OUT / 'REPORT.md', ROOT / 'docs/TESTING-0.26.md']:
    destination.write_text(report, 'utf-8')
(ROOT / 'builds/SHA256SUMS-0.26.0.txt').write_text(''.join(f"{item['sha256']}  {Path(item['path']).name}\n" for item in release['artifacts']), 'utf-8')
for filename in ['minimal-left-393.png', 'wood-left-393.png']:
    shutil.copy2(OUT / 'ui' / filename, ROOT / 'docs/images' / ('evaluation26-' + filename))
