...
$taskOutput = Join-Path $taskRoot 'review/app/chessis19'
New-Item -ItemType Directory -Force -Path $taskOutput | Out-Null
if (-not $UiOnly) {
    $taskCore = if ($RegressionOnly) { @('report12_test', 'story15_test') } else { @('story19_test') }
    foreach ($taskScript in $taskCore) {
        & $taskGodot --headless --path (Join-Path $taskRoot 'godot') --script "res://tests/$taskScript.gd" -- --chessis19-regression *> (Join-Path $taskOutput ($taskScript + '.log'))
        if ($LASTEXITCODE -ne 0) { throw "$taskScript failed." }
        Get-Content -LiteralPath (Join-Path $taskOutput ($taskScript + '.log')) -Tail 1
    }
}
$taskProbes = if ($Probes) { $Probes } elseif ($RegressionOnly) { @('chessis12', 'chessis18', 'chessis16') } else { @('chessis19') }
foreach ($taskProbe in $taskProbes) {
    ...
    $taskProcess = Start-Process -FilePath $taskGodot -ArgumentList @('--path', ('"' + (Join-Path $taskRoot 'godot') + '"'), '--', '--unified-test', ('--' + $taskProbe), '--chessis19-regression') -WindowStyle Hidden -RedirectStandardOutput $taskStdout -RedirectStandardError $taskStderr -PassThru
