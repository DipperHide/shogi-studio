...
func probe_classification_verification() -> void:
	...
	verify(ribbon.find_children("PhaseSummaryFlow", "HFlowContainer", true, false).size() == 2, "packaged summaries wrap independently for both players")
	verify(ribbon.targets.filter(func(item): return item.kind == "rating").size() == 2, "packaged report exposes unavailable rating for both players")
	verify(ribbon.find_child("PhaseMetricAccuracy", true, false).find_child("MetricValue", true, false).text.ends_with("%"), "packaged accuracy retains percentage format")
	app.ui.page_scroll.ensure_control_visible(ribbon)
	await capture("phase-ribbon")
	ribbon.targets.filter(func(item): return item.kind == "rating")[0].button.pressed.emit()
	verify(app.ui.page_name == "report-metric-info", "packaged rating explanation opens without payment gate")
	app.ui.back()
	verify(app.ui.report_selected_ply == 3 and app.game == original, "packaged rating round trip preserves selected move and live game")
	await app.get_tree().process_frame
