...
func palette() -> Dictionary:
	...
	if preferences.appearance != "wood":
		p.merge(Design.flat_palette(dark), true)
		colors.classic = p.board.to_html(false)
...
func _update_font() -> void:
	...
	if preferences.language == "ja": next.fallbacks = [Design.font_with_weight(FONT, 400.0)]
...
func _set_replay(ply: int) -> void:
	...
	# The end of our own match is the live board, not a separate replay state.
	# Imported records and candidate previews keep their independent cursor.
	if review_game == null and replay_index == game.moves.size(): replay_index = -1
	_refresh()
