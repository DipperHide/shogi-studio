...
0.34.0 默认入口使用 `studio-icon.svg` 龙王「龍」图标，浅木色背景与柔和投影。保留已有龍字矢量路径；`scripts/build_studio_icon.gd` 生成 PNG、Android 透明前景与 Windows ICO。自适应前景不包含矩形底色，棋子缩进遮罩安全区，背景独立使用浅木色。页面木纹由渲染代码绘制。没有把参考 Chessis APK 的图标、棋子、角色或木纹位图打入安装包。以下 0.7 素材记录为历史说明，旧资源保留供原有美术场景使用，不再作为 0.8 的主页和玩家头像。
