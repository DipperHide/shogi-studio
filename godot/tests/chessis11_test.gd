...
func run(instance) -> void:
	...
	if "--chessis29-regression" in OS.get_cmdline_user_args(): output = ProjectSettings.globalize_path("res://../review/app/chessis29/chessis11")
	if "--chessis30-regression" in OS.get_cmdline_user_args(): output = ProjectSettings.globalize_path("res://../review/app/chessis30/chessis11")
