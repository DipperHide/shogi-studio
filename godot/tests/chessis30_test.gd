...
func run(instance) -> void:
	...
	check(await until(func(): return not app.ui.archive_view.indexing,8), "archive index finishes")
	check(app.ui.archive_view.entries.size() == 1, "recent picker lists saved games")
	await press("ArchiveLoad_" + saved_path.get_file().sha256_text().left(12))
	check(await until(func(): return not app.ui.archive_view.busy,8), "archive parser finishes")
	await settle()
