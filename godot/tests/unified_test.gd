...
func records_and_engine() -> void:
	...
	app._set_replay(2)
	await settle(0.25)
	app._continue_review()
	if app.ui.page_name == "replace-game": await click("不保存并继续")
	check(app.game.moves.size() == 2, "explicit continue branches at viewed position after save decision")
