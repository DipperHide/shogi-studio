...
0.34.2 明确历史回放的“从这里下”操作；返回回放保留选中的局面，确认续下后恢复落子。见 [验证记录](docs/TESTING-0.34.2.md)。

面向 Android 和 Windows 的免费将棋对弈、复盘与学习应用，使用 Godot 4.7.2 和 YaneuraOu 本地引擎。当前版本 **0.34.2**，**所有功能免费开放，没有会员、购买入口或付费解锁**。

![木制棋盘](docs/images/board-0.20.png)

## 安装

源码和安装包已公开：[0.34.2 Release](https://github.com/DipperHide/shogi-studio/releases/tag/v0.34.2)。每日棋谱更新已启用，并已完成首次云端更新。

从 [Releases](https://github.com/DipperHide/shogi-studio/releases) 下载 `Shogi-0.34.2-android-arm64.apk`，适用于 ARM64 Android 设备。Windows 下载 `Shogi-0.34.2-windows-x64.zip`，解压后运行 `Shogi.exe`，保留同目录的 `engines` 文件夹。
...
- 0.34 持驹改用浅木色托盘，修复姓名与计时重叠；菜单保持棋盘尺寸，补充英文／日文棋盘及设置显示。纯回放不归档，修改后可保存、不保存或继续编辑；文件导入可仅载入。龙王图标使用浅木色背景。见 [保存行为与实机优化](docs/OPTIONAL-SAVING.md)。

[变化分析用法](docs/VARIATIONS.md) · [0.34 测试结果](docs/TESTING-0.34.md) · [Evaluation bar / 評価バー / 评价条](docs/EVALUATION-BAR.md) · [Position editor / 局面編集 / 局面编辑](docs/POSITION-EDITOR.md) · [Opening preview / 戦法プレビュー / 开局预览](docs/OPENING-PREVIEW.md) · [Analysis import / 棋譜入力 / 分析导入](docs/ANALYSIS-IMPORT.md) · [Tournament archive / 大会棋譜 / 大赛棋谱](docs/TOURNAMENT-ARCHIVE.md)
...
./scripts/test_package.ps1 -ReportDirectory review/app/chessis34/package -Executable builds/windows-0.34.2/Shogi.exe
```
